...
void zs_monitor::classify_image(std::vector<uint64_t> l_image) {
   ...
   monitor_activations[0] = image_1d_to_3d(l_image, cnn_kernels[0].num_input_rows, cnn_kernels[0].num_input_columns,
   ...
   for (int layer_idx = 0; layer_idx < cnn_num_layers; layer_idx++) {
      ...
      monitor_activations[layer_idx + 1] = compute_layer(monitor_activations[layer_idx], cnn_kernels[layer_idx]);
...
void zs_monitor::check_layer_activations(std::vector<uint64_t> activations, int layer_idx) {
   ...
   if (error_found == false)
      ...
      log_utilities::error("***Errors detected in layer %d", layer_idx);#include "zs_monitor.cpp"
