...
inline void initAxiMemoryPointers(){
	...
	virtual_address = (unsigned int *)mmap(NULL, 65535, PROT_READ | PROT_WRITE, MAP_SHARED, dh, device_addr_offset); // Memory map AXI Lite register block
	virtual_destination_address = (unsigned int *)mmap(NULL, 65535, PROT_READ | PROT_WRITE, MAP_SHARED, dh, dest_addr_offset); // Memory map destination address
	virtual_source_address  = (unsigned int *)mmap(NULL, 65535, PROT_READ | PROT_WRITE, MAP_SHARED, dh, src_addr_offset); // Memory map source address
}
...
inline int writeAxiCommit(int wordsNumber,unsigned int startPos){
	...
	if(wordsNumber > 0 || wordsNumber <= 64){
		...
		dma_set(virtual_address, MM2S_START_ADDRESS, src_addr_offset+startPos);
...
inline unsigned int dma_set(unsigned int* 	dma_virtual_address, int offset, unsigned int value) {
