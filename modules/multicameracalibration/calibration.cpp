...
void MultiCalibration::updateSettings(MultiCalibrationSettings settings) {
	...
	if (settings->useFisheyeModel_cam0) {
		...
		flag = fisheye::CALIB_FIX_SKEW | fisheye::CALIB_RECOMPUTE_EXTRINSIC
				| fisheye::CALIB_FIX_K2 | fisheye::CALIB_FIX_K3
...
bool MultiCalibration::stereoCalibrate(MultiCalibrationSettings settings) {
	this->settings = settings;
		...
		for (int j = 0; j < this->settings->boardHeigth; j++)
			for (int k = 0; k < this->settings->boardWidth; k++)
				objectPoints[i].push_back(
						Point3f(k * this->settings->boardSquareSize,
								j * this->settings->boardSquareSize, 0));
	}

	double rms = cv::stereoCalibrate(objectPoints, imagePoints_cam0,
			imagePoints_cam1, undistortCameraMatrix_cam0,
			undistortDistCoeffs_cam0, undistortCameraMatrix_cam1,
			undistortDistCoeffs_cam1, imageSize, R, T, E, F,
	...
	for (int i = 0; i < this->settings->numPairsImagesBeforCalib; i++) {
		...
		undistortPoints(imgpt[0], imgpt[0], undistortCameraMatrix_cam0,
				undistortDistCoeffs_cam0, Mat(), undistortCameraMatrix_cam0);
		computeCorrespondEpilines(imgpt[0], 0 + 1, F, lines[0]);

		imgpt[1] = Mat(imagePoints_cam1[i]);
		undistortPoints(imgpt[1], imgpt[1], undistortCameraMatrix_cam1,
				undistortDistCoeffs_cam1, Mat(), undistortCameraMatrix_cam1);
		computeCorrespondEpilines(imgpt[1], 1 + 1, F, lines[1]);

		for (int j = 0; j < npt; j++) {
			double errij = fabs(
					imagePoints_cam0[i][j].x * lines[1][j][0]
							+ imagePoints_cam0[i][j].y * lines[1][j][1]
							+ lines[1][j][2])
					+ fabs(
							imagePoints_cam1[i][j].x * lines[0][j][0]
									+ imagePoints_cam1[i][j].y * lines[0][j][1]
									+ lines[0][j][2]);
			err += errij;
		}
		npoints += npt;
	...
	if (fs.isOpened()) {
		fs << "M1" << undistortCameraMatrix_cam0 << "D1"
				<< undistortDistCoeffs_cam0 << "M2"
				<< undistortCameraMatrix_cam1 << "D2"
				<< undistortDistCoeffs_cam1;
		fs.release();
	} else
	...
	stereoRectify(undistortCameraMatrix_cam0, undistortDistCoeffs_cam0,
			undistortCameraMatrix_cam1, undistortDistCoeffs_cam1, imageSize, R,
			T, R1, R2, P1, P2, Q, CALIB_ZERO_DISPARITY, 1, imageSize,
...
void * MultiCalibration::findNewPoints(caerFrameEvent frame, int camid) {
	if (frame == NULL || !caerFrameEventIsValid(frame)) {
		return (false);
	}

	// Initialize OpenCV Mat based on caerFrameEvent data directly (no image copy).
	Size frameSize(caerFrameEventGetLengthX(frame),
			caerFrameEventGetLengthY(frame));
	Mat orig(frameSize, CV_16UC(caerFrameEventGetChannelNumber(frame)),
			caerFrameEventGetPixelArrayUnsafe(frame));
...
size_t MultiCalibration::foundPoints(int camid) {
	if (camid == 0)
...
bool MultiCalibration::loadCalibrationFile(MultiCalibrationSettings settings) {
	...
	if (!fs["camera_matrix"].empty()
			&& !fs["distortion_coefficients"].empty()) {
		caerLog(CAER_LOG_NOTICE,
				"MultiCalibration CXX loadCalibrationFile() CAM0:",
	...
	if (!fs1["camera_matrix"].empty()
			&& !fs1["distortion_coefficients"].empty()) {
		caerLog(CAER_LOG_NOTICE,
				"MultiCalibration CXX loadCalibrationFile() CAM1:",
				"Camera matrix and distorsion coefficients succesfully loaded");
	} else {
		caerLog(CAER_LOG_ERROR,
				"MultiCalibration CXX loadCalibrationFile() CAM1",
