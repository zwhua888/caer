...
void caerMultiCalibration(uint16_t moduleID, caerPolarityEventPacket polarity, caerFrameEventPacket frame) {
	...
	caerModuleSM(&caerMultiCalibrationFunctions, moduleData, sizeof(struct MultiCalibrationState_struct), 2, polarity,
		frame);
}
...
static void updateSettings(caerModuleData moduleData) {
    ...
    state->settings.doCalibrateStick = sshsNodeGetBool(moduleData->moduleNode, "doCalibrateStick");
